# Superdisign Menu — מדריך פקודות עיצוב
...
### `/superdisign_menu` (לשעבר `/impeccable`)
...
**שימוש:** `/superdisign_menu craft <feature>` / `/superdisign_menu teach` / `/superdisign_menu extract <target>`
...
**פלט:** brief שניתן להעביר ל-`/superdisign_menu craft` או לכל implementation skill.
...
```
/superdisign_menu teach          → context (פעם אחת בתחילת פרויקט)
/shape <feature>                  → design brief
/superdisign_menu craft           → bot build
/audit + /critique               → QA
...
תיקיית `superdisign_menu/reference/`:
...
| קטגוריה | פקודות |
|---|---|
| ראשי | `/superdisign_menu` (3 מצבים: craft/teach/extract) |
| תכנון | `/shape` |
